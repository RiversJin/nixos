"""Daily scenery wallpaper and desktop palette; all state is local and replaceable."""
import argparse
...
OUTPUTS = tuple(os.environ['NIRI_WALLPAPER_OUTPUTS'].split())
PRIMARY = OUTPUTS[0]
RECENT_LIMIT = int(os.environ['NIRI_WALLPAPER_RECENT_LIMIT'])
FILES = tuple(os.environ['NIRI_THEME_FILES'].split())
...
def fetch(url, limit):
    # Use the same bounded curl download path on all hosts.
    data = subprocess.run([
...
def choose_image(state, old, items):
    ...
    # Prefer unseen images; a small collection must not stop working after RECENT_LIMIT picks.
    # Keep all currently displayed images excluded, including during subsequent picks.
