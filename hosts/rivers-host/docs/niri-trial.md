...
每天本地时间 09:00 从 Wallhaven Anime 分类的 landscape/scenery 搜索结果中随机选图，取最近一年 toplist 的前 3 页候选；保留 SFW、至少 3840×2160、16:9 限制。DP-1（下屏）和 DP-3（上屏）各一张且不重复；进入 niri 时补选当天壁纸，同一天自动触发不会重复更换。排除最近 28 张；任一下载失败保留整组旧图，下次登录或手动操作时可以重试。定时器仅跟随 niri 会话运行。
