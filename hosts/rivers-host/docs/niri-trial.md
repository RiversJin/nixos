...
host 与 laptop 共用仓库根目录的 `home/niri-daily.nix` 和 `home/niri-daily.py`。机器配置只传入屏幕列表、历史长度、主题模板与备用图片：host 为 DP-1 / DP-3、28 张历史；laptop 为 eDP-1、14 张历史。图源、下载校验、配色和刷新逻辑统一维护。
